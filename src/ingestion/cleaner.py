...
def clean_department_df(raw_df: pl.DataFrame, catalogs: Catalogs) -> pl.DataFrame:
    ...
    cod_norm = pl.when(is_capital_00).then(
        pl.concat_str([pl.lit("01-"), cod_raw.str.slice(3)])
    ).otherwise(cod_raw).alias("codigo_establecimiento")

    # Extraer municipio_codigo (4 dígitos)
    mupio_code = pl.when(is_capital_00).then(
        pl.lit("0101")
    ).otherwise(
        pl.concat_str([cod_raw.str.slice(0, 2), cod_raw.str.slice(3, 2)])
    ...
    sector_col = (
        pl.col("Sector")
        .replace_strict(catalogs.sector_map, default="Ignorado")
        .alias("sector")
    )
    area_col = (
        pl.col("Área")
        .replace_strict(catalogs.area_map, default="Ignorado")
        .alias("area")
    )
    sexo_col = (
        pl.col("Sexo")
        .replace_strict(catalogs.sexo_map, default="Ignorado")
        .alias("sexo")
    )
    nivel_col = (
        pl.col("Nivel")
        .replace_strict(catalogs.nivel_map, default="Ignorado")
        .alias("nivel")
    )
    pueblo_col = (
        pl.col("Pueblo_Per")
        .replace_strict(catalogs.pueblo_map, default="Ignorado")
        .alias("pueblo_pertenencia")
    )
    plan_col = (
        pl.col("Plan_Est")
        .replace_strict(catalogs.plan_est_map, default="Ignorado")
        .alias("plan_estudios")
    )
    jornada_col = (
        pl.col("Jornada_Est")
        .replace_strict(catalogs.jornada_map, default="Ignorado")
        .alias("jornada")
    )
    resultado_col = (
        pl.col("Resultado_F")
        .replace_strict(catalogs.resultado_map, default="Ignorado")
        .alias("resultado")
    )
    repitente_col = (
        pl.col("Repitente")
        .replace_strict(catalogs.repitente_map, default="Ignorado")
        .alias("repitente")
    )
    graduando_col = (
        pl.col("Graduando")
        .replace_strict(catalogs.graduando_map, default="Ignorado")
        .alias("graduando")
    )
    ...
    dept_map_expr = pl.col("departamento_codigo").replace_strict(
        catalogs.departamentos_map, default="Desconocido"
    ).alias("departamento")

    mupio_map_expr = pl.col("municipio_codigo").replace_strict(
        catalogs.municipios_map, default="Desconocido"
    ).alias("municipio")
